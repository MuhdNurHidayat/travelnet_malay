...
--[[
 ...
 21.06.13 - bugfix: wielding an elevator while digging a door caused the elevator_top to be placed
          - leftover floating elevator_top nodes can be removed by placing a new travelnet_malay:elevator underneath them and removing that afterwards
...
minetest.register_privilege("travelnet_attach", { description = "membolehkan pemain memasukkan stesen travelnet ke dalam jaringan travelnet pemain lain", give_to_singleplayer = false});
minetest.register_privilege("travelnet_remove", { description = "membolehkan pemain menggali stesen travelnet dimiliki oleh pemain lain", give_to_singleplayer = false});
...
dofile(minetest.get_modpath("travelnet_malay").."/config.lua"); -- the normal, default travelnet
...
travelnet.save_data = function()
   ...
   if( file ) then
      ...
      print("[Mod travelnet] Ralat: Fail simpanan '"..tostring( path ).."' tidak boleh ditulis.");
   end
end
...
travelnet.restore_data = function()
   ...
   if( file ) then
      ...
      print("[Mod travelnet] Ralat: Fail simpanan '"..tostring( path ).."' tidak dijumpai.");
   end
end
...
travelnet.update_formspec = function( pos, puncher_name )
   ...
   if( this_node ~= nil and this_node.name == 'travelnet_malay:elevator' ) then
      is_elevator = true;
   end 
...
-- minetest.chat_send_player(puncher_name, "data: "..minetest.serialize(  travelnet.targets ));
      ...
      meta:set_string("infotext",       "Kotak Travelnet (belum ditetapkan)");
      ...
      meta:set_string("formspec", 
                            "size[12,10]"..
                            "field[0.3,7.6;9,0.9;station_name;Nama stesen ini:;"..(station_name or "?").."]"..
                            "field[0.3,8.6;9,0.9;station_network;Masukkan ke Jaringan:;"..(station_network or "?").."]"..
                            "field[0.3,9.6;9,0.9;owner;Dimiliki oleh:;"..(owner_name or "?").."]"..
                            "button_exit[6.3,8.2;1.7,0.7;station_set;Simpan]" );

      minetest.chat_send_player(puncher_name, "Ralat: Gagal mengemaskini! Menetapkan semula kotak ini dalam jaringan travelnet.");
      return;
   end
   ...
   if(  not( travelnet.targets[ owner_name ] )
     ...
     or not( travelnet.targets[ owner_name ][ station_network ][ station_name ] )) then
      ...
      minetest.chat_send_player(owner_name, "Stesen '"..station_name.."' telah dimasukkan semula ke dalam jaringan '"..station_network.."'.");
   ...
   local zusatzstr = "";
   local trheight = "10";
   if( this_node and this_node.name=="locked_travelnet_malay:travelnet" ) then
      zusatzstr = "field[0.3,11;6,0.7;locks_sent_lock_command;Travelnet berkunci. Taip /help untuk bantuan:;]";
      trheight = "11.5";
   end
   local formspec = "size[12,"..trheight.."]"..
                            "label[3.3,0.0;Kotak Travelnet:]".."label[6.3,0.0;Ketuk kotak untuk kemaskini senarai stesen.]"..
                            "label[0.3,0.4;Nama stesen ini:]".."label[6.3,0.4;"..(station_name or "?").."]"..
                            "label[0.3,0.8;Nama jaringan:]" .."label[6.3,0.8;"..(station_network or "?").."]"..
                            "label[0.3,1.2;Nama pemilik:]"            .."label[6.3,1.2;"..(owner_name or "?").."]"..
                            "label[3.3,1.6;Tekan nama stesen untuk pergi situ:]"..
			    zusatzstr;
--                            "button_exit[5.3,0.3;8,0.8;do_update;Ketuk kotak untuk kemaskini senarai stesen. Tekan nama stesen untuk pergi situ.]"..
   local x = 0;
   local y = 0;
   local i = 0;
...
--         formspec = formspec .. ']';
   ...
   meta:set_string( "infotext", "Stesen '"..tostring( station_name ).."' dalam jaringan '"..tostring( station_network )..
                                "' (milik "..tostring( owner_name )..") sedia untuk digunakan. Klik-kanan untuk bergerak, ketuk untuk kemaskini.");

   minetest.chat_send_player(puncher_name, "Senarai destinasi kotak ini telah dikemaskini.");
end
...
travelnet.add_target = function( station_name, network_name, pos, player_name, meta, owner_name )
   ...
   if( this_node.name == 'travelnet_malay:elevator' ) then
--      owner_name   = '*'; -- the owner name is not relevant here
   ...
   if( station_name == "" or not(station_name )) then
      minetest.chat_send_player(player_name, "Sila tulis nama untuk stesen ini.");
      return;
   end

   if( network_name == "" or not( network_name )) then
      minetest.chat_send_player(player_name, "Sila tulis nama jaringan untuk menyambung stesen ini.");
      return;
   end
   ...
   elseif( not( minetest.check_player_privs(player_name, {interact=true}))) then
      ...
      minetest.chat_send_player(player_name, "Pemain '"..tostring( player_name ).."' tidak mempunyai keistimewaan interact. Henti paksa.");
      return;
   ...
   elseif( not( minetest.check_player_privs(player_name, {travelnet_attach=true}))
       ...
       minetest.chat_send_player(player_name, "Anda tidak ada keistimewaan travelnet_attach yang diperlukan untuk memasukkan kotak anda ke dalam jaringan pemain lain. Henti paksa.");
      return;
   end
   ...
   for k,v in pairs( travelnet.targets[ owner_name ][ network_name ] ) do
      ...
      if( k == station_name ) then
         minetest.chat_send_player(player_name, "Ralat: Stesen bernama '"..station_name.."' sudah wujud dalam jaringan ini. Sila pilih nama yang berbeza!");
         return;
      end
   ...
   if( anz+1 > travelnet.MAX_STATIONS_PER_NETWORK ) then
      minetest.chat_send_player(player_name, "Ralat: Jaringan '"..network_name.."' telah mengandungi jumlah stesen maksimum yang dibenarkan (="
              ..(travelnet.MAX_STATIONS_PER_NETWORK).."). Sila pilih nama jaringan yang berbeza atau yang baru.");
      return;
   end
   ...
   if( meta ) then
      ...
      minetest.chat_send_player(player_name, "Stesen '"..station_name.."' telah ditambah ke dalam jaringan '"
                                          ..network_name.."', yang kini mempunyai "..( anz+1 ).." stesen.");
      ...
      meta:set_string("formspec", 
                     "size[12,10]"..
                     "field[0.3,0.6;6,0.7;station_name;Stesen:;"..   meta:get_string("station_name").."]"..
                     "field[0.3,3.6;6,0.7;station_network;Jaringan:;"..meta:get_string("station_network").."]" );
...
travelnet.open_close_door = function( pos, player, mode )
   ...
   if( door_node ~= nil and door_node.name ~= 'ignore' and door_node.name ~= 'air' and minetest.registered_nodes[ door_node.name ] ~= nil and minetest.registered_nodes[ door_node.name ].on_rightclick ~= nil) then
      ...
      if( mode==1 and ( door_node.name == 'travelnet_malay:elevator_door_glass_closed'
                     or door_node.name == 'travelnet_malay:elevator_door_steel_closed' 
                     -- handle doors that change their facedir
                     or ( door_node.param2 == this_node.param2
                      and door_node.name ~= 'travelnet_malay:elevator_door_glass_open'
                      and door_node.name ~= 'travelnet_malay:elevator_door_steel_open'))) then
         return;
      end
      -- do not open the doors if they are already open (works only on elevator-doors; not on doors in general)
      if( mode==2 and ( door_node.name == 'travelnet_malay:elevator_door_glass_open'
                     or door_node.name == 'travelnet_malay:elevator_door_steel_open'
                     -- handle doors that change their facedir
                     or ( door_node.param2 ~= this_node.param2 
                      and door_node.name ~= 'travelnet_malay:elevator_door_glass_closed'
                      and door_node.name ~= 'travelnet_malay:elevator_door_steel_closed'))) then
         return;
      end
...
travelnet.on_receive_fields = function(pos, formname, fields, player)
   ...
   if( not( fields.target )) then
      minetest.chat_send_player(name, "Sila tekan pada destinasi yang anda ingin pergi.");
      return;
   end
   ...
   if(  not( owner_name  ) 
     ...
     or not( travelnet.targets[ owner_name ][ station_network ] )) then
      ...
      if(     owner_name
         ...
         minetest.chat_send_player(name, "Ralat: Ada yang tidak kena dengan tetapan stesen ini. "..
                                      " DATA NYAHPEPIJAT: pemilik: "..(  owner_name or "?")..
                                      ", nama stesen: "..(station_name or "?")..
                                      ", jaringan: "..(station_network or "?")..".");
         return
      end
   end
   ...
   if(  not( owner_name )
     ...
     or not( travelnet.targets[ owner_name ] )
     or not( travelnet.targets[ owner_name ][ station_network ] )) then
      minetest.chat_send_player(name, "Ralat: Travelnet ini tidak cukup data dan/atau ditetapkan dengan cara yang salah.");
      print( "RALAT: Stesen travelnet dekat "..minetest.pos_to_string( pos ).." mempunyai masalah: "..
                                      " DATA: pemilik: "..(  owner_name or "?")..
                                      ", nama stesen: "..(station_name or "?")..
                                      ", rangkaian: "..(station_network or "?")..".");
      return;
   end
   ...
   if( not( travelnet.targets[ owner_name ][ station_network ][ fields.target ] )) then
      ...
      minetest.chat_send_player(name, "Stesen '"..( fields.target or "?").." (sudah?) tidak wujud dalam jaringan ini.");
   ...
   minetest.chat_send_player(name, "Memulakan pergerakan ke stesen '"..( fields.target or "?").."'.'");
   ...
   if( travelnet.travelnet_effect_enabled ) then 
      minetest.add_entity( {x=pos.x,y=pos.y+0.5,z=pos.z}, "travelnet_malay:effect"); -- it self-destructs after 20 turns
   end
   ...
   if( travelnet.travelnet_effect_enabled ) then 
      minetest.add_entity( {x=target_pos.x,y=target_pos.y+0.5,z=target_pos.z}, "travelnet_malay:effect"); -- it self-destructs after 20 turns
   end


   -- check if the box has at the other end has been removed.
   local node2 = minetest.get_node(  target_pos );
   if( node2 ~= nil and node2.name ~= 'ignore' and node2.name ~= 'travelnet_malay:travelnet' and node2.name ~= 'travelnet_malay:elevator' and node2.name ~= "locked_travelnet_malay:travelnet" and node2.name ~= "travelnet_malay:travelnet_private") then
...
travelnet.remove_box = function( pos, oldnode, oldmetadata, digger )
   ...
   if( not( oldmetadata ) or oldmetadata=="nil" or not(oldmetadata.fields)) then
      minetest.chat_send_player( digger:get_player_name(), "Ralat: Tidak jumpa maklumat berkaitan stesen yang ingin dibuang.");
      return;
   end
   ...
   if(  not( owner_name ) 
     ...
     or not( travelnet.targets[ owner_name ][ station_network ] )) then
      ...
      minetest.chat_send_player( digger:get_player_name(), "Ralat: Tidak jumpa stesen yang ingin dibuang.");
      return;
   end
   ...
   -- inform the owner
   minetest.chat_send_player( owner_name, "Stesen '"..station_name.."' telah DIBUANG daripada rangkaian '"..station_network.."'.");
   if( digger ~= nil and owner_name ~= digger:get_player_name() ) then
      minetest.chat_send_player( digger:get_player_name(), "Stesen '"..station_name.."' telah DIBUANG daripada rangkaian '"..station_network.."'.");
   end
...
travelnet.can_dig = function( pos, player, description )
   ...
   if( not( meta ) or not( owner) or owner=='') then
      minetest.chat_send_player(name, description.." ini masih belum ditetapkan. Sila tetapkan dahulu untuk menuntutnya. Kemudian anda boleh membuangnya kerana anda menjadi pemiliknya.");
      return false;

   elseif( owner ~= name ) then
      minetest.chat_send_player(name, description.." ini milik "..tostring( meta:get_string('owner'))..". Anda tidak boleh membuangnya.");
      return false;
   end
   return true;
end
...
if( travelnet.travelnet_effect_enabled ) then
  minetest.register_entity( 'travelnet_malay:effect', {
...
if( travelnet.travelnet_enabled ) then
   dofile(minetest.get_modpath("travelnet_malay").."/travelnet.lua"); -- the travelnet node definition
end
if( travelnet.elevator_enabled ) then
   dofile(minetest.get_modpath("travelnet_malay").."/elevator.lua");  -- allows up/down transfers only
end
if( travelnet.doors_enabled ) then
   dofile(minetest.get_modpath("travelnet_malay").."/doors.lua");     -- doors that open and close automaticly when the travelnet or elevator is used
end

if( travelnet.abm_enabled ) then
   dofile(minetest.get_modpath("travelnet_malay").."/restore_network_via_abm.lua"); -- restore travelnet data when players pass by broken networks
end
